<div>
    ...
    <div class=" px-4 md:px-20 w-full  justify-center items-center flex-col bg-white py-10">
        <x-beardcumb menu='product' submenu='{{ $product->nama_product }}' routemenu='admin.product' />
        <div class="flex justify-center items-start gap-10 mt-6 flex-col md:flex-row">
                ...
                <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                    ...
                    @elseif ($product->fotos->count() === 1)
                        ...
                        <div class="block w-full h-full">
                            <img src="{{ asset($product->fotos[0]->foto) }}" class="w-full h-full object-cover"
                                alt="...">
                        </div>
                    @else
            ...
            <div class="w-full md:w-1/2">
                <h1 class="text-3xl font-semibold mb-2">{{ $product->nama_product }}</h1>
                <h1 class="text-2xl font-semibold mb-1">Rp. {{ number_format($product->harga, 0, ',', '.') }}</h1>
                @if ($product->stok <= 0)
                    <h1 class="font-semibold" style="color: red">Stok: Sedang Kosong</h1>
                @else
                    <h1 class="font-semibold">Stok: {{ $product->stok }}</h1>
                @endif
                <div class="mt-2">
                    <h1 class="text-xl font-medium">Product Deskripsi</h1>
                    {!! $product->deskripsi !!}
                </div>
                <div class="mt-2">
                    <h1 class="text-xl font-medium">Product Spesifikasi</h1>
